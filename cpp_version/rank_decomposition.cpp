...
#pragma omp parallel num_threads(4)
  {
    ...
    for(int i=0; i<simplices.size();++i){
      assert (simplices[i].face.size() <= rank_dim+2);
      std::pair<Simplex_handle, bool> insert_return = st.insert_simplex(simplices[i].face, Filtration_value(i));//initialize the simplex tree
      if(insert_return.second==false)
	cerr << "ERROR:insert fail!!!" << endl;
    }
...
int main(int argc, char* const argv[]) {
    ...
    if(argc!=6){
      cout <<"Usage: " << argv[0] << " input_filename range_x range_y rank_dim output_filename\n"
	   << "where:\n"
	   << "  - range_x is the grid size in x\n"
	   << "  - range_y is the grid size in y\n"
	   << "  - rank_dim is the dimension in homology\n";
        return 0;
    }
    else{
        filename=argv[1];
